...
resource "null_resource" "setup_db" {
  ...
  provisioner "local-exec" {
    command = "psql 'user=${local.rates_api_username} password=${module.rates_rds_password.value} host=${module.rds.rds_writer_internal_endpoint} dbname=${local.rates_api_db_name}' < files/db/rates.sql"
  }
}
