locals {
  ...
  rates_api_name = "${local.env}-rates-api"
}
...
module "ecs_rates_api_container_definition" {
  ...
  version                  = "0.58.1"
  container_name           = local.rates_api_name
...
resource "aws_ecs_service" "rates_api" {
  ...
  depends_on = [module.rates_api_alb, null_resource.rates_api_provisioner, module.ec2_jumphost]
}
