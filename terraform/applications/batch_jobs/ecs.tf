...
module "ecs_rates_batch_jobs_container_definition" {
  ...
  container_image = var.rate_api_image_to_deploy == "" ? "${module.ecr_rates_batch_jobs.repository_url}:${local.ecr_rates_batch_jobs_tag_sha}" : "${module.repository_url.repository_url}:${var.rate_api_image_to_deploy}"
