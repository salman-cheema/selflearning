locals {
  ...
  rates_batch_job_name = "${local.env}-rates-batch-jobs"
}
...
module "ecs_rates_batch_jobs_container_definition" {
  ...
  version                  = "0.58.1"
  container_name           = local.rates_batch_job_name
  ...
  environment = [
    ...
    # It needs to be override while running ecs task
    { name  = "S3_BUCKET"
      value = ""
    },
    # It needs to be override while running ecs task
...
resource "aws_ecs_task_definition" "task_def_rates_batch_jobs" {
  count                    = can(data.terraform_remote_state.rates_api_app.outputs.rds_endpoint) ? 1 : 0
  family                   = local.rates_batch_job_name
